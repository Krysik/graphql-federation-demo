...
func (ec *executionContext) resolveEntity(
	...
	case "Movie":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Movie": %w`, err)
		}
		switch resolverName {
		...
		case "findMovieByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findMovieByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindMovieByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Movie": %w`, err)
			}
	...
	case "Review":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Review": %w`, err)
		}
		switch resolverName {
		...
		case "findReviewByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findReviewByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindReviewByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Review": %w`, err)
			}
...
func entityResolverNameForMovie(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Movie", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Movie", ErrTypeNotFound))
			break
		}
		return "findMovieByID", nil
	}
	return "", fmt.Errorf("%w for Movie due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForReview(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Review", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Review", ErrTypeNotFound))
			break
		}
		return "findReviewByID", nil
	}
	return "", fmt.Errorf("%w for Review due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
